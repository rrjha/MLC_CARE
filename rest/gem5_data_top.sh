...
#	sed '5q;d' 'mix_encodingNLRU_LW'$ds'_r4_trace-'$a'_16M_32way_f0_t12.txt'	| 	awk '{printf "%5.2f\t", $4} END {if(!NR) printf "%9s\t", " "}' 
	...
	do
		...
		grep  "system.l3.tags.total_ranks::1" $a/$ds/stats.txt	| 	awk '{printf "%9.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
		grep  "system.l3.tags.total_ranks::2" $a/$ds/stats.txt	| 	awk '{printf "%9.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
		grep  "system.l3.tags.total_ranks::3" $a/$ds/stats.txt	| 	awk '{printf "%9.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
		grep  "system.l3.tags.total_ranks::4" $a/$ds/stats.txt	| 	awk '{printf "%9.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
		grep  "system.l3.tags.total_ranks::total" $a/$ds/stats.txt	| 	awk '{printf "%9.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
		#grep  "system.l3.tags.total_ranks" $a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
		grep  "system.l3.tags.total_Reps::total" $a/$ds/stats.txt	| 	awk '{printf "%9.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
