...
LIST="bwaves soplex zeusmp GemsFDTD gobmk bzip2 h264ref lbm leslie3d mcf omnetpp perlbench cactusADM hmmer sphinx3 tonto gamess"
LIST="soplex zeusmp GemsFDTD gobmk bzip2 lbm leslie3d mcf omnetpp perlbench cactusADM sphinx3 sjeng"
...
#	done
DESIGN="options_108 options_109 options_110"
#echo "                                       mlc                                        lru"
for a in $LIST
do
	printf "%-15s\t" $a
...
#		sed '19q;d' 'mix_encodingNLRU_LW'$ds'_r4_trace-'$a'_16M_32way_f4_t12.txt'	| 	awk '{printf "%4.4f\t", $9} END {if(!NR) printf "%9s\t", " "}' 
	...
	#	grep  "system.l3.total_trans::3" ../../gem5_twostep/rest/$a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}'
		#grep  "system.switch_cpus_1.ipc_total" ../../gem5_twostep/rest/$a/$ds/stats.txt	| 	awk '{printf "%1.6f\t", $2} END {if(!NR) printf "%9s\t", " "}'
