...
LIST="sphinx3 zeusmp GemsFDTD gobmk bzip2 leslie3d omnetpp cactusADM perlbench soplex lbm mcf sjeng"
#LIST="leslie3d"
	...
	do
		...
		do
			...
			do
				...
				grep  "system.l3.tags.total_Reps::$num " $a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
				#grep  "system.l3.tags.total_ZT::$num " $a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
        			grep  "system.l3.tags.total_ZT::$num " $a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
				grep  "system.l3.tags.total_ST::$num " $a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
				grep  "system.l3.tags.total_HT::$num " $a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}' 
				grep  "system.l3.tags.total_TT::$num " $a/$ds/stats.txt	| 	awk '{printf "%5.0f\t", $2} END {if(!NR) printf "%9s\t", " "}'
				printf "\n"
			done
		done
